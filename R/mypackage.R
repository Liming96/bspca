...
#' }
#' @export
bspca2 <- function(x,k=ncol(x),type=c("Gram","predictor"),lambda=10000,ncomp=min(dim(x)),
  ...
  Q <- matrix(0,p, ncomp)  # orthonormal basis for the principal subspace
  ...
  for(cc in ccs){
    if(type=="predictor"){
      ...
      res <- cov_spca(Sp,sparsity=k[cc],lambda,Q[,seq_len(cc-1),drop=FALSE],
                      bess_tol,bess_maxiter)#计算第cc个主成分
    }
    w <- res$w
    W[,cc] <- w #第cc个主成分为w
    ...
    if (cc > 1) {
      q <- w - Q[ , 1:(cc-1)]%*%(t(Q[ , 1:(cc-1)])%*%w) #对w进行投影
    } else {
      q <- w
    }
    q <- q/sqrt(sum(q^2))
...
pre_spca <- function(Xp,sparsity,lambda,Q,bess_tol=1e-3,bess_maxiter=100){
  n <- nrow(Xp)
  p <- ncol(Xp)
...
cov_spca <- function(Sp,sparsity,lambda,Q,bess_tol=1e-3,bess_maxiter=100){
